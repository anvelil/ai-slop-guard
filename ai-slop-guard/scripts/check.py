...
ai-slop-guard/data/rules.json (see references/registry.md for the full list).

Standard library only. No network calls. No files are written or installed.

Usage:
    python3 check.py <file_or_dir> [<file_or_dir> ...]
    python3 check.py --json <file_or_dir> [<file_or_dir> ...]
    python3 check.py --explain ASG002
...
RULES_JSON = Path(__file__).resolve().parent.parent / "data" / "rules.json"
...
# Put this comment on a line to suppress any finding on that line, e.g. a
# CLI tool's own legitimate print() output:
#   print("done")  # slop-guard: ignore
...
def _out(*args, **kwargs) -> None:
    """CLI output. Deliberately not named print() so this tool's own
    ASG007 check does not flag its own command-line output -- the
    same reasoning a project applies when it routes through a logger
    instead of print(), just without needing a logging dependency."""
    print(*args, **kwargs)  # slop-guard: ignore -- the one real print() call, deliberately wrapped above
...
def check_python(path: Path, source: str) -> list[Finding]:
    ...
    # --- ASG007: leftover debug output (AST-based: real print() calls only) ---
    for node in ast.walk(tree):
        if (
    ...
    # --- ASG002: file-local candidate dead functions ---
    # Module-level, undecorated functions only, excluding pytest's test_*
    # naming convention. Both exclusions exist for the same reason class
    # methods are excluded: a decorated function (@app.route, @click.command,
    # @pytest.fixture) is registered with a framework and invoked indirectly,
    # never by name in this file — and neither is a test_* function, which
    # pytest discovers by name rather than by call. A benchmark run against
    # Flask's own tutorial example (see benchmarks/) originally flagged
    # every single route handler and test function as "dead" before this
    # exclusion was added — see docs/known-limitations.md.
    defined = {}
    ...
    # A name applied as a decorator (`@foo`, without parens) is a use too —
    # it doesn't show up as an ast.Call the way `foo()` does.
    for node in ast.walk(tree):
...
def main(argv: list[str]) -> int:
    if "--explain" in argv:
        ...
        if idx + 1 >= len(argv):
            _out("Usage: check.py --explain ASGxxx")
            return 2
